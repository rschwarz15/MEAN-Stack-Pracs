...
module.exports = {
    ...
    getAllWithinYears: function (req, res) {
        ...
        if (!isNaN(req.params.year1) && !isNaN(req.params.year2)) {
            let endYear = +req.params.year1;
            let startYear = +req.params.year2;

            // Check that the numbers are integers
            if (Number.isInteger(endYear) && Number.isInteger(startYear)) {
                if (endYear <= startYear) return res.status(400).json("Year1 must be greater than Year2")

                let filter = { "year": { $lte: endYear, $gte: startYear } };
    ...
    deleteAllWithinYears: function (req, res) {
        ...
        if (!isNaN(req.body.year1) && !isNaN(req.body.year2)) {
            let endYear = +req.body.year1;
            let startYear = +req.body.year2;
    ...
    deleteAllWithinYearsURLParams: function (req, res) {
        // Check the strings are numbers
        if (!isNaN(req.params.year1) && !isNaN(req.params.year2)) {
            let endYear = +req.params.year1;
            let startYear = +req.params.year2;
